...
def init_callbacks(dash_app):
    ...
    def login_result(n_clicks, username, password):
        ...
        if username is None or password is None: return html.P(children=["Usuario o contraseña equivocado, por favor vuelve a intentar."], style={"color":"red", 'margin-left':'auto','margin-right':'auto', 'margin-top':'5px'})
        login_result = loginizer(username, password)
        #print(login_result)
        if login_result == False: return html.P(children=["Usuario o contraseña equivocado, por favor vuelve a intentar."], style={"color":"red", 'margin-left':'auto','margin-right':'auto', 'margin-top':'5px'})
        else:
